...
public:
  ...
  const std::streampos EndianTestBegin =
    132;                                // Position where binary portion of file begins
  ...
  : Filename(filename)
{
  ...
  try {
    ...
    infile >> scratch >> floatSizeTest >> Nx1 >> Ny1 >> Nz1 >> Nelt >> Nelgt >> Time >>
      Iostep >> Fid0 >> Nfileoo >> Rdcode >> P0th >> pressMeshKey;
  } catch (std::ifstream::failure& e) {
    std::cerr << e.what()
              << ": Couldn't open or read fld.file in ASCII mode: " << Filename
              << std::endl;
    throw e;
  }
  ...
  if (floatSizeTest != sizeof(FloatT)) {
    throw std::runtime_error("Error: .fld file " + Filename + " has " +
                             std::to_string(floatSizeTest) +
                             "-byte floats, but the FldReader was instantiated to read " +
...
std::vector<std::string> FldHeader<FloatT, IntT>::RdcodeTokens() const
{
  ...
  std::regex_iterator<std::string::iterator> regIter(
    sequence.begin(), sequence.end(), pattern);
