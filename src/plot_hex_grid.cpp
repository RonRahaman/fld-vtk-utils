...
int main(int argc, char* argv[])
{
  ...
  auto gridEnd = tick();
  std::cout << "Grid setup time: " << tick_diff(gridStart, gridEnd) / 1e9 << " s"
            << std::endl;
  ...
  renWin->Render();
  auto renderEnd = tick();
  std::cout << "Rendering time: " << tick_diff(renderStart, renderEnd) / 1e9 << " s"
            << std::endl;
  std::cout << "Total time: " << tick_diff(totStart, renderEnd) / 1e9 << " s"
            << std::endl;
